...
while runLoop
    ...
    if size(p0, 1) <= 3
        ...
        if ~isempty(faces)
            ...
            roi_gray = imcrop(img_gray,faces);
            %imshow(roi_gray);
            %%-your-code-ends-here-%%
            ...
            %img=insertMarker(img,points.Location+[faces(1) faces(2)],'color','red')
            %imshow(img)
            % Convert from ROI to image coordinates
            %%-your-code-starts-here-%%
            p0 = p0+[faces(1) faces(2)];  % replace with you implementation
            %img=insertMarker(img,p0)
            %%-your-code-ends-here-%%
	    ...
	    % from p1.      
        %%-your-code-starts-here-%%
        something=p1(isFound,:);
        %%-your-code-ends-here-%%
        
        % Draw points using e.g. insertMarker
        %%-your-code-starts-here-%%
        img=insertMarker(img,something,'color','yellow');
        %%-your-code-ends-here-%%
